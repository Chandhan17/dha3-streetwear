...
async function post(path, body) {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      throw new Error(`Network request failed for ${endpoint}. Check API URL, TLS certificate, CORS, and backend availability.`)
    }
    throw error
  }
}
...
export const verifyPayment = async (orderId, paymentId, signature) => post('/api/verify-payment', {
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          ...
          const completed = await post('/api/online/complete-order', {
      ...
      modal: {
        ondismiss: () => onFailure?.(new Error('Payment modal closed')),
      },
    }
