...
function normalizeProduct(productId, productData) {
  ...
  return { id: productId, ...productData, name: normalizeText(productData.name), price: normalizePrice(productData.price), purchasePrice: normalizePrice(productData.purchasePrice), mrp: normalizePrice(productData.mrp), gstPercent: normalizeNumber(productData.gstPercent), hsnCode: normalizeText(productData.hsnCode), sku: normalizeText(productData.sku), barcode: normalizeText(productData.barcode), stock: normalizeNumber(productData.stock), openingStock: normalizeNumber(productData.openingStock), openingStockValue: normalizeNumber(productData.openingStockValue), category: normalizeCategory(productData.category), sizes: normalizeSizes(productData.sizes), description: normalizeText(typeof productData.description === 'object' ? productData.description?.short : productData.description || productData.shortDescription), sizeChartText: normalizeText(productData.sizeChartText), sizeChartImage: normalizeText(productData.sizeChartImage), images: normalizeImageList(productData.images, imageUrl), image: imageUrl, imageUrl }
}
...
function buildProductPayload(data, imageUrl, existing = {}) {
  ...
  return { name: normalizeText(name), price: normalizePrice(price), category: normalizeCategory(category), sizes: normalizeSizes(sizes), description: normalizeText(description), sizeChartText: normalizeText(sizeChartText), sizeChartImage: normalizeText(sizeChartImage), images: normalizedImages, image: primaryImage, imageUrl: primaryImage, sku: normalizeText(sku) || existing.sku || generateSku(name), barcode: normalizeText(barcode) || existing.barcode || generateBarcode(), purchasePrice: normalizePrice(purchasePrice), mrp: normalizePrice(mrp || price), gstPercent: normalizeNumber(gstPercent), hsnCode: normalizeText(hsnCode), stock: normalizeNumber(stock ?? existing.stock), openingStock: normalizeNumber(openingStock ?? existing.openingStock ?? normalizedOpeningStock), openingStockValue: normalizeNumber(openingStockValue ?? existing.openingStockValue ?? normalizedOpeningStock * normalizePrice(purchasePrice)) }
}
