...
function normalizeDiscountPercent(value) { const number = Number(value); return Number.isFinite(number) ? Math.min(100, Math.max(0, Math.round(number * 100) / 100)) : 0 }
function getDiscountedPrice(price, discountPercent) { const basePrice = normalizePrice(price); const discount = normalizeDiscountPercent(discountPercent); return Math.round(basePrice * (1 - discount / 100) * 100) / 100 }
...
function buildProductPayload(data, imageUrl, existing = {}) {
  const { name, price, category, sizes, description, sizeChartText, sizeChartImage, images, sku, barcode, purchasePrice, mrp, gstPercent, hsnCode, stock, openingStock, openingStockValue, discountPercent } = data
  const normalizedSizes = normalizeSizes(sizes)
...
export async function createProduct({ name, price, discountPercent, category, sizes, description, sizeChartText, sizeChartImage, imageFiles, imageFile, sku, barcode, purchasePrice, mrp, gstPercent, hsnCode, openingStock = 0, openingStockValue, allowMissingImage = false }) {
  ...
  const normalizedSizes = normalizeSizes(sizes)
  const payload = buildProductPayload({ name, price, discountPercent, category, sizes: normalizedSizes, description, sizeChartText, sizeChartImage, images: uploadedImages, sku, barcode, purchasePrice, mrp, gstPercent, hsnCode, stock: normalizedSizes.length ? normalizedSizes.length : openingStock, openingStock: normalizedSizes.length ? normalizedSizes.length : openingStock, openingStockValue }, imageUrl)
  const ref = await addDoc(productsCollectionRef, payload); invalidateProductsCache(); return { id: ref.id, ...payload }
}
export async function editProduct(productId, { name, price, discountPercent, category, sizes, description, sizeChartText, sizeChartImage, imageFiles, imageFile, currentImages, currentImageUrl, currentImage, sku, barcode, purchasePrice, mrp, gstPercent, hsnCode, openingStock, openingStockValue, stock }) { const existing = await fetchProductById(productId); if (!existing) throw new Error('Product not found.'); let nextImages = Array.isArray(currentImages) ? normalizeImageList(currentImages) : normalizeImageList([], currentImageUrl || currentImage); if (Array.isArray(imageFiles) && imageFiles.length) nextImages = [...nextImages, ...(await uploadImages(imageFiles))]; else if (imageFile) nextImages = [...nextImages, await uploadImage(imageFile)]; const normalizedEditSizes = normalizeSizes(sizes); const payload = buildProductPayload({ name, price, discountPercent, category, sizes: normalizedEditSizes, description, sizeChartText, sizeChartImage, images: nextImages, sku, barcode, purchasePrice, mrp, gstPercent, hsnCode, openingStock, openingStockValue, stock: normalizedEditSizes.length ? existing.stock : openingStock, sizeStock: existing.sizeStock }, nextImages[0] || '', existing); if (!normalizedEditSizes.length && (stock === undefined || stock === null) && (openingStock === undefined || openingStock === null)) payload.stock = existing.stock; if (!normalizedEditSizes.length && (openingStock === undefined || openingStock === null)) payload.openingStock = existing.openingStock; if (openingStockValue === undefined || openingStockValue === null) payload.openingStockValue = existing.openingStockValue; await updateDoc(doc(db, PRODUCTS_COLLECTION, productId), payload); invalidateProductsCache(); return payload }
