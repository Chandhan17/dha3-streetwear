  ...
  return (
    <AdminLayout activeKey="analytics" onChangeKey={handleNavigation} title="Reports & Analytics" query="" onQueryChange={() => {}} onLogout={async () => navigate('/login', { replace: true })}>
      <div className="space-y-6">
        <div><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#c19a6b]">Business reporting</p><h1 className="mt-1 text-2xl font-semibold text-white">Sales & Profit Reports</h1><p className="mt-1 text-sm text-white/50">POS reports are queried from Firestore using the selected date range, so the report is not limited to the latest 200 bills.</p></div>
        ...
        <section className="rounded-2xl border border-white/10 bg-[#111111] p-5 shadow-soft">
          ...
          {ordersLoading ? <div className="mt-5"><Loader label="Loading order history" /></div> : orderHistoryRows.length === 0 ? <div className="mt-5"><EmptyState title="No orders" description="Try another date range." /></div> : <div className="mt-5 overflow-x-auto rounded-xl border border-white/10"><table className="w-full min-w-[1200px] text-left text-sm"><thead><tr className="border-b border-white/10 text-[11px] uppercase tracking-[0.1em] text-white/45"><th className="px-4 py-3">Date</th><th className="px-4 py-3">Order ID</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Mobile</th><th className="px-4 py-3">Products</th><th className="px-4 py-3">Quantity</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Payment Method</th><th className="px-4 py-3">Payment Status</th><th className="px-4 py-3">Order Status</th><th className="px-4 py-3">Source</th></tr></thead><tbody>{orderHistoryRows.map((row) => <tr key={row.orderId} className="border-b border-white/5"><td className="px-4 py-3 text-white/60">{row.date}</td><td className="px-4 py-3 text-white/70">{row.orderId}</td><td className="px-4 py-3 text-white">{row.customer}</td><td className="px-4 py-3 text-white/50">{row.mobile}</td><td className="max-w-[340px] px-4 py-3 text-white/70">{row.products}</td><td className="px-4 py-3 text-white/70">{row.quantity}</td><td className="px-4 py-3 text-[#f0ddc4]">{formatCurrency(row.amount)}</td><td className="px-4 py-3 text-white/60">{row.paymentMethod}</td><td className="px-4 py-3 text-white/60">{row.paymentStatus}</td><td className="px-4 py-3 text-white/60">{row.orderStatus}</td><td className="px-4 py-3"><Badge tone="success">Online</Badge></td></tr>)}</tbody></table></div>}
        </section>
      </div>
    </AdminLayout>
  )
