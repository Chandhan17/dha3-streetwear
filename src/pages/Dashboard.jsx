      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-[#111111] p-2">
            {[['today', 'Today'], ['month', 'This Month'], ['all', 'All Time']].map(([value, label]) => <button key={value} type="button" onClick={() => setPeriod(value)} className={`rounded-xl px-4 py-2 text-xs font-semibold uppercase tracking-[0.12em] transition ${period === value ? 'bg-[#c19a6b] text-black' : 'text-white/60 hover:bg-white/5 hover:text-white'}`}>{label}</button>)}
        ...
        {isLoading ? <Loader label="Loading dashboard" /> : <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><StatCard label="Sales" value={currency(metrics.sales)} hint={period === 'today' ? 'Today' : period === 'month' ? 'This month' : 'All time'} /><StatCard label="Cost" value={currency(metrics.cost)} hint="Product cost" /><StatCard label="Profit" value={metrics.profit === null ? '—' : currency(metrics.profit)} hint={metrics.profitKnown ? 'Sales minus product cost' : 'Some older orders need cost data'} /><StatCard label="Margin" value={metrics.margin === null ? '—' : `${metrics.margin.toFixed(2)}%`} hint="Profit ÷ sales" /></div>
