...
function POS() {
  ...
  const addToCart = (product) => {
    ...
    setCart((current) => {
      ...
      return [...current, { id: product.id, name: product.name, price: Number(product.price || 0), quantity: 1, barcode: product.barcode || '', sku: product.sku || '', stock }]
    })
  }
  ...
  const handleCompleteSale = async () => {
    ...
    setMessage('')
    try {
      const response = await completePOSSale({ items: cart.map((item) => ({ productId: item.id, quantity: item.quantity })), paymentMethod, discount })
