...
function POS() {
  ...
  const addProductToCart = (product, selectedSize = 'N/A') => {
    ...
    setCart((current) => {
      ...
      return [...current, { cartItemId, id: product.id, name: product.name, price: Number(product.effectivePrice ?? product.price ?? 0), quantity: 1, barcode: product.barcode || '', sku: product.sku || '', stock, gstPercent: Number(product.gstPercent ?? product.gst ?? 0), selectedSize }]
    })
