...
function AdminLayout({ activeKey, onChangeKey, title, query, onQueryChange, onLogout, isLoggingOut = false, children }) {
  ...
  const [isLoggingOutInternally, setIsLoggingOutInternally] = useState(false)
  ...
  const menuItems = useMemo(() => [
    ...
    { key: 'orders', label: 'Orders', icon: <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M8 7h13M8 12h13M8 17h13M3 7h.01M3 12h.01M3 17h.01" strokeLinecap="round" /></svg> },
  ...
  const handleLogout = async () => {
    setIsLoggingOutInternally(true)
    try {
      await logoutAdmin()
      ...
      setIsLoggingOutInternally(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className={`min-h-screen transition-all duration-300 ${collapsed ? 'md:pl-[84px]' : 'md:pl-[248px]'}`}>
        <Topbar title={title} query={query} onQueryChange={onQueryChange} onLogout={handleLogout} onOpenMobileSidebar={() => setMobileOpen(true)} isLoggingOut={isLoggingOutInternally || isLoggingOut} />
